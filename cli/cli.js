...
// Self-heal SQLite runtime deps (sql.js + better-sqlite3) into ~/.0Router/runtime
...
// Identifiers for killAllAppProcesses - only kill 0Router specifically
const PROCESS_IDENTIFIERS = [
  '0Router'  // Only package name - avoid killing other apps
];
    ...
    try {
      ...
      if (platform === "win32") {
        ...
        try {
          ...
          const lines = output.split("\n").slice(1).filter(l => l.trim());
          lines.forEach(line => {
            // Whitelist: real node process running 0Router/cli.js, or next-server.
            // Avoids killing editors/grep/strace/cursor that just have "0Router" in cmdline.
            const cmd = line.toLowerCase();
            const isAppProcess =
              (cmd.includes("node") && cmd.includes("0Router") && (cmd.includes("cli.js") || cmd.includes("\\0Router") || cmd.includes("/0Router")))
        ...
        try {
          ...
          lines.forEach(line => {
            // Whitelist: real node process running 0Router/cli.js, or next-server.
            // Avoids killing grep/strace/editors/cursor that incidentally match "0Router".
            const cmd = line.toLowerCase();
            const isAppProcess =
              (cmd.includes("node") && cmd.includes("0Router") && (cmd.includes("cli.js") || cmd.includes("/0Router")))
              || cmd.includes("next-server");
            if (isAppProcess) {
              const parts = line.trim().split(/\s+/);
              const pid = parts[1];
...
function startServer(latestVersion) {
      ...
      while (true) {
        ...
        if (choice === "update") {
          ...
          if (process.platform === "darwin") {
            ...
            console.log(`🔔 0Router is running in tray (PID: ${process.pid})`);
            console.log(`   Server: http://${displayHost}:${port}`);
          ...
          console.log(`🔔 0Router is now running in background (PID: ${bgProcess.pid})`);
          console.log(`   Server: http://${displayHost}:${port}`);
          console.log(`\n💡 You can close this terminal. Right-click tray icon to quit.\n`);
  ...
  function tryRestart(code) {
    ...
    if (restartCount >= MAX_RESTARTS) {
      ...
      try {
        const dbPath = path.join(os.homedir(), process.platform === "win32" ? path.join("AppData", "Roaming", "0Router", "db.json") : path.join(".0Router", "db.json"));
