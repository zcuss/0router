...
<div align="center">
  ...
  **将所有 AI 编程工具（Claude Code、Cursor、Antigravity、Copilot、Codex、Gemini、OpenCode、Cline、OpenClaw...）连接到 40+ AI 提供商和 100+ 模型。**
  ...
  [🚀 快速开始](#-快速开始) • [💡 功能特点](#-主要功能) • [📖 设置指南](#-设置指南) • [🌐 网站](https://0Router.com)
...
本仓库的包是私有的（`0Router-app`），所以源码/Docker 执行是预期的本地开发方式。
...
<table>
  <tr>
    <td align="center" width="320">
      ...
      <b>🇺🇸 English</b><br/>
      <sub>0Router + Claude Code 免费设置<br/>by <a href="https://www.youtube.com/@BuildAIWithHamid">Build AI With Hamid</a></sub>
    </td>
    <td align="center" width="320">
      ...
      <b>🇻🇳 Tiếng Việt</b><br/>
      <sub>使用 0Router 节省 OpenClaw 的 LLM 成本<br/>by <a href="https://www.youtube.com/c/M%C3%ACAIblog">Mì AI</a></sub>
    </td>
    <td align="center" width="320">
  ...
  <tr>
    <td align="center" width="320">
      ...
      <b>🇺🇸 English</b><br/>
      <sub>使用 0Router 免费设置 Claude CLI 🚀<br/>by <a href="https://www.youtube.com/@CodeVerseSoban">CodeVerse Soban</a></sub>
    </td>
    <td align="center" width="320">
      ...
      <b>🇻🇳 Tiếng Việt</b><br/>
      <sub>从零开始安装 OpenClaw 免费版 + 0Router<br/>by <a href="https://www.youtube.com/@maigia">Mai Gia</a></sub>
    </td>
    <td align="center" width="320">
...
> 🎬 **制作了关于 0Router 的视频？** 提交 [Pull Request](https://github.com/decolua/0Router/pulls)，将你的视频添加到此部分 — 我们会合并它！

---

## 🛠️ 支持的 CLI 工具

0Router 与所有主流 AI 编程工具无缝协作：
...
- 你的 CLI 工具发送 OpenAI 格式 → 0Router 转换 → 提供商接收原生格式
...
> 0Router 本身**永远不会向你收费**。你只直接向提供商付款（如果使用付费服务）。
> 
> **示例：** 如果你的控制面板显示使用 iFlow 模型时"总成本 $290"，这代表你如果直接使用付费 API 需要支付的金额。你的实际成本 = **$0**（iFlow 免费无限量）。
> 
> 把它想象成一个"节省追踪器"，展示你通过使用免费模型或通过 0Router 路由节省了多少钱！
...
- **低价提供商**（GLM、MiniMax）：直接付款，0Router 只做路由
- **免费提供商**（iFlow、Kiro、Qwen）：真正的永久免费，无隐藏费用
- **0Router**：从不收取任何费用，永远不会
...
控制面板追踪你的 token 使用情况，并显示**估算成本**，如同你直接使用付费 API。这**不是实际计费** — 它是一个参考，展示你通过使用免费模型或通过 0Router 路由现有订阅节省了多少钱。
...
<details>
<summary><b>💳 0Router 会扣我的钱吗？</b></summary>

**不会。** 0Router 是在你自己的电脑上运行的开源软件。它永远不会向你收取任何费用。

**你只需支付：**
- ✅ **订阅提供商**（Claude Code $20/月、Codex $20-200/月）→ 在他们的网站上直接付款
- ✅ **低价提供商**（GLM、MiniMax）→ 直接付款，0Router 只是路由你的请求
- ❌ **0Router 本身** → **永不收费，永远不会**

0Router 是一个本地代理/路由器。它没有你的信用卡，不能发送发票，也没有计费系统。它是完全免费的软件。
...
0Router 只是路由你的请求到它们 — 没有"陷阱"或未来的计费。它们是真正的免费服务，0Router 让它们易于使用并支持切换。
...
**你掌控一切：** 在控制面板中设置每个提供商的支出限制，0Router 会遵守它们。
...
**专业提示：** 复杂任务使用 Opus，追求速度使用 Sonnet。0Router 按模型追踪配额！
...
设置 → 模型 → 高级：
  ...
  OpenAI API Key：[来自 0Router 控制面板]
...
> **注意：** OpenClaw 仅适用于本地 0Router。使用 `127.0.0.1` 而不是 `localhost` 以避免 IPv6 解析问题。
...
| 变量 | 默认值 | 描述 |
|----------|---------|-------------|
| `JWT_SECRET` | 自动生成（`~/.0Router/jwt-secret`） | 用于控制面板 auth cookie 的 JWT 签名密钥（设置可在多实例间共享） |
| `INITIAL_PASSWORD` | `123456` | 当没有保存的哈希时首次登录的密码 |
| `DATA_DIR` | `~/.0Router` | 主应用数据库位置（`db.json`） |
...
| `CLOUD_URL` | `https://0Router.com` | 服务端云同步端点基础 URL |
| `NEXT_PUBLIC_BASE_URL` | `http://localhost:3000` | 向后兼容/公开基础 URL（服务端运行时优先使用 `BASE_URL`） |
| `NEXT_PUBLIC_CLOUD_URL` | `https://0Router.com` | 向后兼容/公开云 URL（服务端运行时优先使用 `CLOUD_URL`） |
...
- `${DATA_DIR}` 和 `~/.0Router` 在 Docker 容器中解析到同一位置 — 符号链接 `/root/.0Router -> /app/data` 在构建时创建。
...
- **网站**：[0Router.com](https://0Router.com)
- **GitHub**：[github.com/decolua/0Router](https://github.com/decolua/0Router)
- **问题**：[github.com/decolua/0Router/issues](https://github.com/decolua/0Router/issues)

---

## 👥 贡献者

感谢所有帮助改进 0Router 的贡献者！
...
**[OmniRoute](https://github.com/diegosouzapw/OmniRoute)** — 0Router 的全功能 TypeScript 分支。增加了 36+ 提供商、4 层自动切换、多模态 API（图像、嵌入、音频、TTS）、断路器、语义缓存、LLM 评估和精美的控制面板。368+ 单元测试。可通过 npm 和 Docker 使用。
...
- **[RTK](https://github.com/rtk-ai/rtk)** ![Stars](https://img.shields.io/github/stars/rtk-ai/rtk?style=flat&color=yellow) — Rust token 节省器。0Router 将其压缩管道移植到 JS → 每次请求 **减少 20-40% 输入 tokens**。
- **[Caveman](https://github.com/JuliusBrussee/caveman)** ![Stars](https://img.shields.io/github/stars/JuliusBrussee/caveman?style=flat&color=yellow) by **[@JuliusBrussee](https://github.com/JuliusBrussee)** — 病毒式传播的 *"为什么用很多 token 当少的 token 就能搞定"*。0Router 适配其提示词 → **减少 65% 输出 tokens**。

非常感谢这些作者 — 没有他们的工作，0Router 的 token 节省功能就不会存在。在 GitHub 上给他们加星！
