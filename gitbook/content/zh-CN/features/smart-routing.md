...
0Router 通过 3 层回退系统,自动将你的请求路由到最佳可用提供商。绝不再因配额限制或速率限制而中断编码。

---

## 工作原理

0Router 使用智能路由,最大化已有订阅价值、最小化成本,并保障 24/7 可用:

```
请求 → 0Router → 检查第 1 层 (订阅)
                     ↓ 配额耗尽
                     检查第 2 层 (低价)
                     ↓ 预算上限
                     检查第 3 层 (免费)
