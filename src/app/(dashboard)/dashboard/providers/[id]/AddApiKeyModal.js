...
export default function AddApiKeyModal({ isOpen, provider, providerName, isCompatible, isAnthropic, authType, authHint, website, proxyPools, error, onSave, onBulkDone, onClose }) {
  ...
  const isCodexAccessToken = provider === "codex" && !isCookie;
  const credentialLabel = isCookie ? "Cookie Value" : (isCodexAccessToken ? "Access Token" : "API Key");
  const credentialPlaceholder = isCookie
    ? (provider === "grok-web" ? "sso=xxxxx... or just the raw value" : "eyJhbGciOi...")
    : (isCodexAccessToken ? "abcdefg.eyJ..." : (isXaiApiKey ? "xai-..." : ""));
