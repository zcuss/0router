...
// Check if settings has 0Router customModels
const has0RouterConfig = (settings) => {
  if (!settings || !settings.customModels) return false;
  return settings.customModels.some(m => m.id?.startsWith("custom:0Router"));
};
...
// POST - Update 0Router customModels (merge with existing settings)
...
export async function POST(request) {
  try {
    ...
    // Remove all existing 0Router configs
    settings.customModels = settings.customModels.filter(m => !m.id?.startsWith("custom:0Router"));
...
// DELETE - Remove 0Router customModels only (keep other settings)
