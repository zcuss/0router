...
const has0RouterConfig = (config) => {
  ...
  return config.some((entry) => entry.name === "0Router");
};

const get0RouterEntry = (config) => {
  if (!Array.isArray(config)) return null;
  return config.find((entry) => entry.name === "0Router") || null;
};
...
export async function GET() {
  try {
    ...
    const entry = get0RouterEntry(config);
...
// POST - Apply 0Router config to chatLanguageModels.json
export async function POST(request) {
  try {
    ...
    // Replace existing 0Router entry or append
    const idx = config.findIndex((e) => e.name === "0Router");
    if (idx >= 0) {
      config[idx] = newEntry;
    } else {
...
// DELETE - Remove 0Router entry from chatLanguageModels.json
export async function DELETE() {
  try {
    ...
    config = config.filter((e) => e.name !== "0Router");
    await fs.writeFile(configPath, JSON.stringify(config, null, 2));

    return NextResponse.json({
      success: true,
      message: "0Router removed from Copilot config",
    });
  } catch (error) {
