...
const has0RouterConfig = (config) => {
  ...
  return !!config.provider["0Router"];
};
...
export async function GET() {
  try {
    ...
    const providerConfig = config?.provider?.["0Router"];
    const modelMap = providerConfig?.models || {};

    return NextResponse.json({
      installed: true,
      config,
      has0Router: has0RouterConfig(config),
      configPath: getConfigPath(),
        opencode: {
          models: Object.keys(modelMap),
          activeModel: config?.model?.startsWith("0Router/") ? config.model.replace(/^0Router\//, "") : null,
...
// POST - Apply 0Router as openai-compatible provider (multi-model support)
export async function POST(request) {
  try {
    ...
    const keyToUse = apiKey || "sk_0Router";
    const effectiveSubagentModel = subagentModel || modelsArray[0];

    // Ensure provider object
    if (!config.provider) config.provider = {};

    // Preserve any existing 0Router provider entry and its models
    const existingProvider = config.provider["0Router"] || { npm: "@ai-sdk/openai-compatible", options: {}, models: {} };
    ...
    config.provider["0Router"] = existingProvider;
    ...
    if (activeModel === "") {
      ...
      if (finalActive) {
        config.model = `0Router/${finalActive}`;
      }
    }
...
export async function PATCH(request) {
  try {
    ...
    if (clearActiveModel === true) {
      ...
      if (config.model?.startsWith("0Router/")) {
        config.model = "";
      }
    }
...
// DELETE - Remove 0Router provider or specific models from config
export async function DELETE(request) {
  try {
    ...
    if (modelToRemove && config.provider?.["0Router"]?.models) {
      delete config.provider["0Router"].models[modelToRemove];
      
      // If no models left, remove the provider
      if (Object.keys(config.provider["0Router"].models).length === 0) {
        delete config.provider["0Router"];
        if (config.model?.startsWith("0Router/")) delete config.model;
      } else if (config.model === `0Router/${modelToRemove}`) {
        // If removed model was active, switch to first remaining model
        const remainingModels = Object.keys(config.provider["0Router"].models);
        config.model = `0Router/${remainingModels[0]}`;
      }
    } else {
      // No specific model - remove entire 0Router provider
      if (config.provider) delete config.provider["0Router"];
      if (config.model?.startsWith("0Router/")) delete config.model;
    }

    // Remove subagent configuration
    if (config.agent?.explorer?.model?.startsWith("0Router/")) {
    ...
    return NextResponse.json({
      success: true,
      message: modelToRemove ? `Model "${modelToRemove}" removed` : "0Router settings removed from OpenCode",
    });
  } catch (error) {
