...
const has0RouterConfig = (auth) => {
  if (!auth) return false;
  const entry = auth["openai-compatible"] || auth["0Router"];
  if (!entry) return false;
  const baseUrl = entry.baseUrl || entry.baseURL || "";
  return baseUrl.includes("localhost") || baseUrl.includes("127.0.0.1") || baseUrl.includes("0Router");
};
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      has0Router: has0RouterConfig(auth),
...
export async function POST(request) {
  try {
    ...
    try {
      ...
      vscode["kilocode.customProvider"] = { name: "0Router", baseURL: normalizedBaseUrl, apiKey };
...
export async function DELETE() {
  try {
    ...
    return NextResponse.json({ success: true, message: "0Router settings removed from Kilo Code" });
  } catch (error) {
