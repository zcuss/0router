...
// (prevents EBUSY on `npm i -g 0Router@latest` while MITM is running).
...
const ENCRYPT_SALT = "0Router-mitm-pwd";
...
async function startServer(apiKey, sudoPassword, forceKillPort443 = false) {
  ...
  if (!effectiveServerPath || !fs.existsSync(effectiveServerPath)) {
    ...
    if (!effectiveServerPath || !fs.existsSync(effectiveServerPath)) {
      throw new Error(`MITM server.js not found at ${effectiveServerPath}. Reinstall 0Router.`);
    }
  }
