...
<div align="center">
  <img src="../images/0Router.png?1" alt="Bảng điều khiển 0Router" width="800"/>
  
  # 0Router - Free AI Router
  ...
  **Nhà cung cấp AI Miễn cho OpenClaw.**
  ...
  [🚀 Bắt đầu nhanh](#-quick-start) • [💡 Tính năng](#-key-features) • [📖 Cài đặt](#-setup-guide) • [🌐 Website](https://0Router.com)
</div>
...
Gói kho lưu trữ này là riêng tư (`0Router-app`), vì vậy việc thực thi nguồn/Docker là đường dẫn phát triển cục bộ dự kiến.
...
### 📺 Hướng dẫn thiết lập hoàn chỉnh - 0Router + Claude Code MIỄN PHÍ
  
[![Thiết lập 0Router + Claude Code](https://img.youtube.com/vi/raEyZPg5xE0/maxresdefault.jpg)](https://www.youtube.com/watch?v=raEyZPg5xE0)

**🎬 Xem hướng dẫn từng đầy đủ:**
- ✅ Cài đặt & thiết lập 0Router
...
0Router hoạt động liền mạch với tất cả các công cụ code AI chính:
...
- Công cụ CLI của bạn gửi định dạng OpenAI → 0Router dịch chuyển → Nhà cung cấp nhận định dạng gốc
...
> Bản thân 0Router **không bao giờ thu phí** bạn bất cứ thứ gì. Bạn chỉ trả tiền trực tiếp cho các nhà cung cấp (nếu sử dụng dịch vụ trả phí).
> 
...
### 📊 Hiểu về Chi phí & Thanh toán của 0Router

**Thực tế Thanh toán 0Router:**

✅ **Phần mềm 0Router = MIỄN PHÍ mãi mãi** (mã nguồn mở, không bao giờ thu phí)  
...
❌ **0Router không bao giờ gửi hóa đơn** hoặc tính phí thẻ của bạn
...
- **Nhà cung cấp giá rẻ** (GLM, MiniMax): Trả tiền trực tiếp cho họ, 0Router chỉ định tuyến
...
<details>
<summary><b>💳 Tôi có bị 0Router tính phí không?</b></summary>

**Không.** 0Router là phần mềm miễn phí, mã nguồn mở chạy trên máy tính của chính bạn. Nó không bao giờ tính phí bạn bất cứ thứ gì.

**Bạn chỉ trả tiền:**
- ✅ **Nhà cung cấp gói đăng ký** (Claude Code $20/tháng, Codex $20-200/tháng) → Trả tiền trực tiếp cho họ trên website của họ
- ✅ **Nhà cung cấp giá rẻ** (GLM, MiniMax) → Trả tiền trực tiếp cho họ, 0Router chỉ định tuyến yêu cầu của bạn
- ❌ **Bản thân 0Router** → **Không bao giờ tính phí bất cứ thứ gì, ever**

0Router là một proxy/router cục bộ. Nó không cóẻ tín dụng của bạn, không thể gửi hóa đơn và không có hệ thống thanh toán. Đó là phần mềm hoàn toàn miễn phí.
...
0Router chỉ định tuyến yêu cầu của bạn đến họ - không có "cạm bẫy" hay thanh toán trong tương lai. Đó là các dịch vụ thực sự miễn phí, và 0Router giúp chúng dễ sử dụng với hỗ trợ dự phòng.

**Lưu ý:** số nhà cung cấp gói đăng ký (Antigravity, GitHub Copilot) có thể có các khoảng thời gian dùng thử miễn phí có thể trở thành trả phí sau này, nhưng điều này sẽ được các nhà cung cấp đó thông báo rõ ràng, không phải 0Router.
...
3. **Sử dụng nhà cung cấp gói đăng ký cuối cùng:**
   ...
   - 0Router giúp tối đa hóa giá trị của chúng thông qua theo dõi hạn mức
...
Cơ chế dự phòng thông minh của 0Router ngăn chặn các khoản phí bất ngờ:
...
**Bạn nắm quyền kiểm soát:** Đặt giới hạn chi tiêu cho mỗi nhà cung cấp trong bảng điều khiển, và 0Router sẽ tuân thủ chúng.
...
**Mẹo Chuyên nghiệp:** Sử dụng Opus cho các tác vụ phức tạp, Sonnet cho tốc độ. 0Router theo dõi hạn mức cho mỗi mô hình!
...
Settings → Models → Advanced:
  ...
  OpenAI API Key: [từ bảng điều khiển 0Router]
...
> **Lưu ý:** OpenClaw chỉ hoạt động với 0Router cục bộ. Sử dụng `127.0.0.1` thay vì `localhost` để tránh các vấn đề phân giải6.
...
| Biến | Mặc định | Mô tả |
|----------|---------|-------------|
| `JWT_SECRET` | Tự động sinh (`~/.0Router/jwt-secret`) | Bí mật ký JWT cho cookie xác thực bảng điều khiển (đặt để chia sẻ giữa nhiều instance) |
| `INITIAL_PASSWORD` | `123456` | Mật khẩu đăng nhập đầu tiên khi không có hash đã lưu tồn tại |
| `DATA_DIR` | `~/.0Router` |ị trí cơ sở dữ liệu ứng dụng chính (`db.json`) |
...
| `CLOUD_URL` | `https://0Router.com` | URL cơ sở endpoint đồng bộ đám mây phía máy chủ |
| `NEXT_PUBLIC_BASE_URL` | `http://localhost:3000` | URL cơ sở tương thích ngược/công khai (ưu tiên `BASE_URL` cho runtime máy chủ) |
| `NEXT_PUBLIC_CLOUD_URL` | `https://0Router.com` | URL đám mây tương thích ngược/công khai (ưu tiên `CLOUD_URL` cho runtime máy chủ) |
...
- Lịch sử sử dụng và log: `~/.0Router/usage.json` và `~/.0Router/log.txt`, được quản lý bởi `src/lib/usageDb.js`.
- request/translator tùy chọn: `<repo>/logs/...` khi `ENABLE_REQUEST_LOGS=true`.
- Lưu trữ sử dụng hiện tại tuân theo logic đường dẫn `~/.0Router` và độc lập với `DATA_DIR`.
...
- Xác minh `CLOUD_URL` trỏ đến endpoint đám mây dự kiến của bạn (ví dụ: `https://0Router.com`)
...
- Triệu chứng thường xuất hiện trên endpoint đám mây công khai (`https://0Router.com/v1`) cho các lệnh gọi không phát trực tiếp (non-streaming).
...
- Runtime 0Router cục bộ bao gồm dự phòng SSE→JSON cho các lệnh gọi không phát trực tiếp khi upstream trả về `text/event-stream`.
...
- `tester/security/test-cloud-openai-compatible.sh`
  - Gửi một yêu cầu tương thích OpenAI trực tiếp đến endpoint đám mây (`https://0Router.com/v1/chat/completions`) với mô hình/key được cung cấp.
...
- Endpoint đám mây công khai (`https://0Router.com/v1/chat/completions`):
...
- **GitHub**: [github.com/decolua/9](https://github.com/decolua/0Router)
...
Cảm ơn tất cả những người đã đóng góp giúp 0Router tốt hơn!

[![Contributors](https://contrib.rocks/image?repo=decolua/0Router&max=100&columns=20&anon=1)](https://github.com/decolua/0Router/graphs/contributors)

---

## 📊 Star Chart

[![ Chart](https://starchart.cc/decolua/0Router.svg?variant=adaptive)](https://starchart.cc/decolua/0Router)
