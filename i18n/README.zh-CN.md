<div align="center">
  ...
  **OpenClaw 的免费 AI 提供商。**
  ...
  [🚀 快速开始](#-quick-start) • [💡 特性](#-key-features) • [📖 设置](#-setup) • [🌐 网站](https://0Router.com)
</div>
...
此仓库包是私有的（`0Router-app`），因此源码/Docker 执行是预期的本地开发路径。
...
- 您的 CLI 工具发送 OpenAI 格式 → 0Router 转换 → 提供商接收原生格式
...
> 0Router 本身**从不向您收费**。您只需直接向提供商付款（如果使用付费服务）。
> 
...
- **廉价提供商**（GLM, MiniMax）：直接向他们付款，0Router 只是路由
...
仪表板追踪您的 Token 使用情况，并显示**估算成本**，就像您直接使用付费 API 一样。这**不是实际计费** - 它是一个参考，显示您通过 0Router 使用免费模型或现有订阅节省了多少。
...
<details>
<summary><b>💳 0Router 会向我收费吗？</b></summary>

**不会。** 9 是免费的开源软件，在您自己的计算机上运行。它从不向您收费。

**您只需支付：**
- ✅ **订阅提供商**（Claude Code $20/月, Codex $20-200/月）→ 在他们的网站上直接向他们付款
- ✅ **廉价提供商**（GLM, MiniMax）→ 直接向他们付款，0Router 只是路由您的请求
- ❌ **0Router 本身** → **从不收取任何费用，永远**

0Router 是本地代理/路由器。它没有您的信用卡，不能发送发票，也没有计费系统。完全免费的软件。
...
Router 只是将您的请求路由到它们 - 没有“陷阱”或未来计费。它们是真正的免费服务，0Router 使它们易于使用并支持回退。

**注意：** 一些订阅提供商（Antigravity, GitHub Copilot）可能有免费预览期，后来可能变成付费，但这会由这些提供商明确宣布，而不是 0Router。
...
**您在控制中：** 在仪表板中设置每个提供商的支出限制，0Router 会遵守它们。
...
**专业提示：** 使用 Opus 处理复杂任务，Sonnet 追求速度。0Router 追踪每个模型的配额！
...
| 变量 | 默认值 | 描述 |
|----------|---------|-------------|
| `JWT_SECRET` | 自动生成（`~/.0Router/jwt-secret`） | 仪表板认证 cookie 的 JWT 签名密钥（设置可在多实例间共享） |
...
| `CLOUD_URL` | `https://0Router.com` | 服务器端云同步端点基础 URL |
| `NEXT_PUBLIC_BASE_URL` | `http://localhost:3000` | 向后兼容/公共基础 URL（服务器运行时优先使用 `BASE_URL`） |
| `NEXT_PUBLIC_CLOUD_URL` | `https://0Router.com` | 向后兼容/公共云 URL（服务器运行时优先使用 `CLOUD_URL`） |
...
- 使用历史和日志：`~/.0Router/usage.json` 和 `~/.0Router/log.txt`，由 `src/lib/usageDb.js` 管理。
...
- 验证 `CLOUD_URL` 指向您预期的云端端点（例如：`https://0Router.com`）
- 尽可能保持 `NEXT_PUBLIC_*` 值与服务器端值一致。

**云端端点 `stream=false` 返回 500（`Unexpected token 'd'...`）**
- 症状通常出现在公共云端端点（`https://0Router.com/v1`）的非流式调用上。
- 根本原因：上游返回 SSE 负载（`data: ...`）而客户端期望 JSON。
- 变通方法：对云端直接调用使用 `stream=true`。
- 当上游返回 `text/event-stream` 时，本地 0Router 运行时包含 SSE→JSON 回退用于非流式调用。
...
- `tester/security/test-cloud-openai-compatible.sh`
  - 使用提供的模型/密钥向云端端点（`https://0Router.com/v1/chat/completions`）发送直接的 OpenAI 兼容请求。
...
- 公共云端端点（`https://0Router.com/v1/chat/completions`）：
...
详细指南请参阅 [Pull Requests](https://github.com/decolua/0Router/pulls)。

---

## 🔀 分支

**[OmniRoute](https://github.com/diegosouzapw/OmniRoute)** — 0Router 的全功能 TypeScript 分支。添加了 36+ 提供商、4 层自动回退、多模态 API（图像、嵌入、音频、TTS）、熔断器、语义缓存、LLM 评估和精美的仪表板。8+ 单元测试。通过 npm 和 Docker 可用。
