...
## Fixes
- CLI build: locate nested Next.js standalone output reliably before npmjs publish
...
## Features
- Codex provider: add dual auth option on `/dashboard/providers/codex` with two choices (`OAuth` and `Access Token`)
- Codex manual credential flow now uses `Access Token` labeling in provider detail modal/UI

## Fixes
- Providers dashboard: Codex card now detects active `access_token` connections so provider is not shown as disabled when token auth is active
- Quota dashboard: include Codex `access_token` connections in provider/quota eligibility and usage checks
- Usage API: allow Codex `access_token` connections to fetch usage/quota (not only OAuth)
- OAuth: fix login flow on Windows

## Improvements
- Rename npm package branding to `0router` in package metadata and CLI-facing labels
